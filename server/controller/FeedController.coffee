...
module.exports =
  index: (req) ->
    ...
    for location in req.user.location
    ...
    .then (tags) ->
      ...
      # nearby online users
      if req.user.coords and req.user.coords.length
        User.find
          coords:
            $nearSphere : req.user.coords
            # $maxDistance : 5 / 6400
          _id:
            $ne: req.user._id
          # online: yes
    ...
    .then (locationNear) ->
      ...
      # _.difference (), [String ]
      # console.log "userIdList", userIdList, [req.user._id]
      User.find
        _id:
          $in: userIdList
          $nin: [req.user._id]
    .then (userList) ->
      for user in userList
        ...
        dist = distance.calculate req.user.coords, user.coords
        if dist
      ...
      # locationIdList = _.difference (), req.user.location
    ...
    .then (locationList) ->
      for location in locationList
        ...
        dist = distance.calculate req.user.coords, location.coords
